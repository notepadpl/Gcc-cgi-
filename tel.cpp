    ...
    EM_ASM(
        {
            ...
            button.addEventListener('click', function() {
                ...
                // Przekazujemy obie wartości do funkcji C++
                _YourCppFunction(hostname, port);
            });
        }
    );
...
extern "C" {
    void YourCppFunction(const char* hostname, int port) {
        printf("Podano host: %s, port: %d\n", hostname, port);
          char* ip_address = resolve_hostname(hostname);
        // Tutaj możesz dodać kod do nawiązania połączenia sieciowego lub innych operacji
    }
}
  struct timeval ts;
